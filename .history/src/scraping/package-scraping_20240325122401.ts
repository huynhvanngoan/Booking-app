...
export const startPackageScraping = async (page: Page, pkg: PackageType) => {
    ...
    const packageDetails = await page.evaluate(() => {
        ...
        const descriptionSelector =
            packageElement?.querySelector("#pkgOverView");
        ...
        packageDetails.description = packageElement
            ...
            ?.innerHTML.replace(regex, "Gre'sBooking") as string;
        ...
        articles?.forEach((article) => {
            ...
            daysSelector.forEach((daySelector) => {
                const activityElements =
                    daySelector.querySelectorAll(".items-content");
                const activities: {
                    activityType: string;
                    activityDescription: string;
                }[] = [];
                ...
                if (activityElements.length > 0) {
                    ...
                    activityElements.forEach((activityElement, index) => {
                        ...
                        if (
                            ...
                            const listHolder =
                                activityElement.querySelector(".list-holder");

                            // Check if the list-holder element exists
                            if (listHolder) {
                                // Extract li elements
                                const liElements =
                                    listHolder.querySelectorAll("li.ng-scope");
                            ...
                            const activityDescriptionElement =
                            ...
                            const places =
                                activityElement.querySelectorAll(
                                    ".place span.full"
                                );
