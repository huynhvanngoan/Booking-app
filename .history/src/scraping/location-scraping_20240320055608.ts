    ...
    return await page.evaluate(() => {
        ...
        packageElements.forEach((packageElement) => {
            ...
            const durationElement =
                packageElement.querySelector(".package-duration");
            ...
            const inclusionsItems = Array.from(
                inclusionsElement?.querySelectorAll("li") || []
            ).map(
                (item) =>
                    (item.querySelector(".icon-name") as HTMLLIElement)
                        ?.textContent || ""
            );

            packageInfo.inclusions = inclusionsItems;

            const priceElement = packageElement.querySelector(
                ".final-price .amount"
            );
