...
export default function Trips() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const data = await apiClient.get(USER_API_ROUTES.GET_ALL_TRIPS)
            if(data.data.trips ) {
        ...
        switch (columnKey) {
            ...
            case "destinationItinerary": {
                ...
                if(Array.isArray(cellValue)) {
                    const intenary = cellValue.slice(0, 4) as { place: string} []
                    return (
                        <div className="flex gap-2">
                            {
                                ...
                                <Chip
                                    key={value.place}
                                    color={`${colors[currentIndex++ % colors.length]}` as  
                                        |"primary" 
                                        |"secondary"
                                        |"success" 
                                        |"warning" 
