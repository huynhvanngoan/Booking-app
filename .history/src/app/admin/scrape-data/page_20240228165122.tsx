...
const ScrapeData = () => {
    ...
    const searchCites = async (searchString: string) => {
        const response = await axios.get(`https://secure.geonames.org/searchJSON?q=${searchString}&maxRows=5&username=ngoan&style=SHORT`);
                ...
                <CardBody>
                    <Tabs>
                        <Tab key='location' title='Location'>
                            <Input type='text' label='Search for a location' onChange={e => searchCites(e.target.value) } />
                        </Tab>
                    </Tabs>
                </CardBody>
